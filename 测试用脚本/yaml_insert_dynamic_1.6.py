# 创建时间: 2025/9/30 12:26
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
process_story_md.py (v1.6)
...
- 在故事标题下方的首个 "> location: xxx" 或 ">地点：xxx" 会被抽取为 collection_place，并从正文删除。
...
CATEGORY_MAP_PATH = r"D:\pythonprojects\folktales\data\mappings\category_map.json"
ETHNIC_MAP_PATH = r"D:\pythonprojects\folktales\data\mappings\ethnic_map.json"
...
def parse_markdown(md_text: str, category_map: Dict[str, str]) -> Tuple[str, List[Section]]:
    ...
    while i < n:
        ...
        if m:
            ...
            # Extract collection_place: support "> location: xxx" and ">地点：xxx"
            collection_place = None
            new_body = []
            loc_pat = re.compile(r"^\s*>\s*(?:location|地点)\s*[:：]\s*(.+?)\s*$", re.IGNORECASE)
            for bl in body:
                pm = loc_pat.match(bl)
                if pm and collection_place is None:  # 只提取第一个匹配项
                    collection_place = pm.group(1).strip()
                new_body.append(bl)  # 保留所有行包括匹配到的行
...
def build_yaml_for_section(
    ...
    *,
    ...
    if is_preface:
        ...
        cat_path_cn = sec.categories_cn or []
        cat_codes = sec.categories_code or []
        category_root = cat_codes[0] if cat_codes else "general"
        category_leaf = cat_codes[-1] if cat_codes else "general"
        category_depth = len(cat_codes) if cat_codes else 1
